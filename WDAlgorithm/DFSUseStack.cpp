...
//		算法过程大致如下：从第一个顶点开始进行深度优先遍历，同时置访问位为true，然后将该顶点入栈，采用while循环，依次取出栈顶元素，
//		并判断该顶点元素是否已访问，未访问则访问，已访问则查看它的firstEdge，当firstEdge也被访问了，查看next，如此循环。
//*/
//#include <stdio.h>
//#include <stdlib.h>
...
//			for (p = G->adjlist[v].firstEdge; p; p = G->adjlist[p->index].firstEdge) {
//				if (!visited[p->index]) {
//					push(s,p->index);
//					visited[p->index] = 1;
//					
